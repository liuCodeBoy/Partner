...
class MyHomePageBasicInfomationContainerViewController: UIViewController, UITableViewDelegate, UITableViewDataSource,PhotoBrowserDelegate  {
...
extension MyHomePageBasicInfomationContainerViewController {
    fileprivate   func loadStatuses() {
        ...
        NetWorkTool.shareInstance.getMyHomePageMomentList(token: access_token, userId: 0, pageNum: pageNum) { [weak self](result, error) in
            if error != nil {
                return }
            NVActivityIndicatorPresenter.sharedInstance.stopAnimating()
            if  result?["code"] as? Int == 200  {
