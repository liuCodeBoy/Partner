...
//    退出圈子（circle/withdrawCircle）
   ...
   //邀请加入圈子 – 圈子创建者（circle/inviteJoin）
    ...
    // 举报（report/report）
    func report(token:String , repoType : Int , reaId : Int, finished:@escaping (_ result : [String : AnyObject]? ,_ error:Error?) ->()){
        //1.获取请求的URLString
        let urlString = "http://47.97.110.89/qm/report/api/report.do"
        //2.获取请求参数
        let parameters = ["token" : token ,"repoType" : repoType , "reaId" : reaId,"id" :repoType] as [String : Any]
        //3.发送请求参数
        request(.POST , urlString: urlString, parameters: parameters as [String : AnyObject]) { (result, error) -> () in
            //获取字典数据
            guard let resultDict = result as? [String : AnyObject] else {
